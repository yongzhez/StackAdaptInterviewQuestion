...
const setDisplayList = createAction("table/SET_DISPLAY");
export const removeFromDisplayList = createAction(
  "table/REMOVE_FROM_DISPLAY_LIST"
);
  ...
  return (dispatch) => {
    requestHandler
      ...
      .then((resp) => {
        dispatch(
          setDisplayList(
            formatQuotes(resp.data.data).sort((a, b) => a.rank - b.rank)
          )
        );
      })
...
const table = createReducer(INITIAL_STATE, (builder) => {
  builder
    ...
    .addCase(removeFromDisplayList, (state, action) => {
      state.displayList = state.displayList.filter(
        (displayed) => displayed.symbol !== action.payload
      );
    })
